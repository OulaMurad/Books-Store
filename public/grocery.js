...
$$("#tab2").on("tab:show", () => {
    ...
    firebase.database().ref("groceries/" + sUser).on("value", (snapshot) =>{
        ...
        for(let n = 0; n < aKeys.length; n++){
         ...
         sCardP .innerHTML= `
            <div class="card">
            <div class="card-content card-content-padding">${oItems[aKeys[n]].author}<div class=\"row\"><div class=\"col\">
            ...
            <div class="card">
            <div class="card-content card-content-padding">${oItems[aKeys[n]].title}</div>
            </div>
            `
            $$("#groceryList").append(sCardP);
        }
...
function createDeleteHandlers(){
    ...
    for(var n = 0; n < aClassname.length; n++){
      aClassname[n].addEventListener("click", (evt) =>{
        ...
        firebase.database().ref("groceries/"+sUser+"/"+ sId).remove();
      })
    }
}
function createFinishHandlers(){
    ...
    for(var n = 0; n < aClassname.length; n++){
      aClassname[n].addEventListener("click", (evt) =>{
        ...
        firebase.database().ref("groceries/"+sUser+"/"+ sId + "/completed").set(sFinished);
      })
    }
}